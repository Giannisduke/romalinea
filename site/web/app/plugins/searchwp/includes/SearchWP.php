...
class SearchWP {
	...
	function __construct() {
		...
		add_action( 'switch_blog', function( $new_blog_id, $prev_blog_id ) {
			if (
				...
				&& apply_filters( 'searchwp\auto_update_providers', true, [ 'site' => $new_blog_id, ] )
			) {
				$this->set_providers();
			}
		}, 99, 2 );
	...
	private static function setup_integrations() {
		if ( apply_filters( 'searchwp\integration\divi',
			'divi' === strtolower( get_template() ) ) )
		{
			$divi = new \SearchWP\Integrations\Divi();
			$divi->modify_native_behavior();
		}

		if ( apply_filters( 'searchwp\integration\beaver-builder',
			'bb-theme' === strtolower( get_template() ) ) )
		{
			$beaver_builder = new \SearchWP\Integrations\BeaverBuilder();
			$beaver_builder->modify_native_behavior();
		}
	}
