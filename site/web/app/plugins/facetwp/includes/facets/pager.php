...
class FacetWP_Facet_Pager extends FacetWP_Facet
{
    ...
    function render( $params ) {
        ...
        if ( method_exists( $this, $method ) ) {
            ...
            if ( 'numbers' == $pager_type ) {
                $output = '<nav class="navbar" aria-label="Page navigation example">' . $output . '</nav>';
            }
    ...
    function render_numbers( $facet ) {
        ...
        if ( 1 < $total_pages ) {
          ...
          if ( 1 < $page && '' != $prev_label ) {
              $output .= $this->render_page_button( $page - 1, $prev_label, 'prev' );
          } else {
            $output .= '<div class="page-item  prev"><a class="page-link prev unactive"> </a></div>';
          }
          $output .= '<ul class="facetwp-pager pagination pagination-lg">';
          ...
          $output .= $this->render_page_button( $page + 1, $next_label, 'next' );
        } else {
          $output .= '<div class="page-item  next"><a class="page-link next unactive"> </a></div>';
        }
    ...
    function render_page_button( $page, $label = false, $extra_class = false ) {
        $label = ( false === $label ) ? $page : $label;
        $class = 'facetwp-page page-link';

        if ( ! empty( $extra_class ) ) {
            $class_parent = ' ' . $extra_class;
        }
        ...
        return '<div class="page-item ' . $class_parent . '"><a class="' . $class . $class_parent . '"' . $data . '>' . $label . '</a></div>';
    }
    ...
    function render_load_more( $facet ) {
        ...
        $output = '<button class="facetwp-load-more btn btn-custom-outline btn-lg btn-block" data-loading="' . esc_attr( $loading_text ) . '"><i class="btn-label"></i>' . esc_attr( $text ) . '</button>';
        return $output;
    }
    ...
    function render_per_page( $facet ) {
        ...
        $output = '<select class="facetwp-per-page-select custom-select custom-select-lg my-3">';
